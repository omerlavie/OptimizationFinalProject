...
class SolveTSPUsingACO:
    ...
    def __init__(self, mode='ACS', colony_size=10, elitist_weight=1.0, min_scaling_factor=0.001, alpha=1.0, beta=3.0,
        ...
        for i in range(self.num_nodes):
            for j in range(i + 1, self.num_nodes):
                self.edges[i][j] = self.edges[j][i] = self.Edge(i, j, nodes[i].weight_to(nodes[j], bop), initial_pheromone)
    ...
    def plot(self, line_width=1, point_radius=math.sqrt(2.0), annotation_size=8, dpi=120, save=True, name=None):
        x = [self.nodes[i].x for i in self.global_best_tour]
        x.append(x[0])
        ...
        for i in self.global_best_tour:
            plt.annotate(self.labels[i], (self.nodes[i].x, self.nodes[i].y), size=annotation_size)
        if save:
            if name is None:
